...
function getDescription(pattern) {
	...
	var description = JSON.stringify(pattern);
	...
	var _check = function _check(value, pattern) {
		...
		if (typeof pattern === 'undefined') {
			...
			return _check(value, Object) && Object.keys(pattern).every(function (key) {
				...
				if (!isValid) {
					throw new _error2.default('Value ' + JSON.stringify(value[key]) + ' is not of type ' + getDescription(pattern[key]), nextKeys.join('.'));
				}
